...
function RouteComponent() {
      ...
      try {
        ...
        const enriched = (data.items ?? [])
          ...
          .map((item) => {
            ...
            return {
              ...base,
              numericValue: Number.isFinite(numericValue)
                ? numericValue
                : undefined,
            }
          })

        let itemsToShow: Array<MeasurementDisplay> = []
        ...
        if (enriched.length) {
          ...
          if (numericValues.length) {
            ...
            setHeartStats({
              ...
              bloodPressure: 'N/A',
              lastCheck: enriched[0]?.date
                ? `${enriched[0].date} ${enriched[0].time}`
                : placeholder,
            })
          } else {
            setHeartStats({
        ...
        if (latest?.value) {
          ...
          setHeartStats({
            restingRate: Number.isFinite(numericValue)
              ? `${numericValue} BPM`
              : placeholder,
            maxRate: Number.isFinite(numericValue)
              ? `${numericValue} BPM`
              : placeholder,
            bloodPressure: 'N/A',
            lastCheck: latestDisplay.date
              ? `${latestDisplay.date} ${latestDisplay.time}`
              : placeholder,
          })
  ...
  const personalInfo = useMemo(() => {
    const height =
      formatNumberWithUnit(user?.height, ' cm') ?? dossierData.personal.height
    const weight =
      formatNumberWithUnit(user?.weight, ' kg') ?? dossierData.personal.weight
  ...
  const heartInfo = useMemo(() => {
    return {
      restingRate:
        heartStats.restingRate || dossierData.heart.restingRate || placeholder,
      maxRate: heartStats.maxRate || dossierData.heart.maxRate || placeholder,
      bloodPressure:
        heartStats.bloodPressure ||
        dossierData.heart.bloodPressure ||
        placeholder,
      lastCheck:
        heartStats.lastCheck ||
        lastMeasurementDate ||
        dossierData.heart.lastCheck ||
        placeholder,
    }
  }, [dossierData.heart, heartStats, lastMeasurementDate])

  const recentMeasurements: Array<MeasurementDisplay> = measurementItems.length
    ? measurementItems
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 px-4 sm:px-8 pb-16">
        ...
        <div className="mt-8 border-b border-white/20 text-white text-center font-['Consolas'] text-3xl font-bold max-w-6xl mx-auto">
          Mijn Dossier
        </div>
          ...
          <Card className="bg-white/10 border-white/20 hover:bg-white/20 hover:scale-[1.01] transition-all backdrop-blur-md h-full">
            ...
            <CardContent>
              <div className="mt-2 space-y-1">
                <StatRow
                  label="Naam:"
                  value={<span>{personalInfo.fullName}</span>}
                />
                <StatRow
                  label="Telefoon:"
                  value={<span>{personalInfo.phone}</span>}
                />
                <StatRow
                  label="Leeftijd:"
                  value={<span>{personalInfo.age}</span>}
                />
                <StatRow
                  label="Geboortedatum:"
                  value={<span>{personalInfo.birthDate}</span>}
                />
                <StatRow
                  label="Lengte:"
                  value={<span>{personalInfo.height}</span>}
                />
                <StatRow
                  label="Gewicht:"
                  value={<span>{personalInfo.weight}</span>}
                />
                <StatRow
                  label="Bloedgroep:"
                  value={<span>{personalInfo.bloodType}</span>}
                />
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/10 border-white/20 hover:bg-white/20 hover:scale-[1.01] transition-all backdrop-blur-md h-full">
            ...
            <CardContent>
              <div className="mt-2 space-y-1">
                <StatRow
                  label="Rusthartslag:"
                  value={<span>{heartInfo.restingRate}</span>}
                />
                <StatRow
                  label="Max hartslag:"
                  value={<span>{heartInfo.maxRate}</span>}
                />
                <StatRow
                  label="Bloeddruk:"
                  value={<span>{heartInfo.bloodPressure}</span>}
                />
                <StatRow
                  label="Laatste controle:"
                  value={<span>{heartInfo.lastCheck}</span>}
                />
              </div>
            </CardContent>
          </Card>
                    ...
                    <div className="flex items-center justify-between bg-black/20 rounded-xl p-4 hover:bg-black/30 transition-colors border border-white/5">
                      <div>
                        ...
                        <div className="text-xs text-white/60">
                          {measurement.time || placeholder}
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="text-sm text-white/80">
                          {measurement.label || placeholder}
                        </div>
